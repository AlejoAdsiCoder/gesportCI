<?php
    class Mreserva extends CI_Model {
        ...
        public function getReservas($ent = '') {
            ...
            if($ent == null) {
                $registros = mysqli_query($con, "select reserva.id AS id, c.nombre AS club, e.nombre AS escenario, reserva.descripcion AS title, fecha_hora_inicio AS start, fecha_hora_fin AS end, reserva.estado AS state, e.foto AS imagen, e.direccion AS dir FROM reserva INNER JOIN club AS c ON reserva.club_id = c.id INNER JOIN escenario AS e ON reserva.escenario_id = e.id") or
